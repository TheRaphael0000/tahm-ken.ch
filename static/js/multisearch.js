...
let search_summoners = (clean_summoners) => {
  let url = '/multisearch/' + select_region.value + '/' + clean_summoners;
...
text_area_multisearch.addEventListener('paste', (event) => {
  ...
  if (summoners_names.size === 5) {
    search_summoners(summoners_names.join(',').replaceAll('#', '-'));
  }
});

btn_multisearch.addEventListener('click', function () {
  search_summoners(
    text_area_multisearch.value
      .split('\n')
